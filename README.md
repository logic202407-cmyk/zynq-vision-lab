...
面向 2026 年全国大学生嵌入式芯片与系统设计竞赛 FPGA 创新设计赛道 AMD 自主选题初级组。本仓库当前公开项目[规划](docs/project_plan.md)、[Gate 执行方案](docs/execution_plan_2026-09-24.md)、[参赛仓库交付计划](docs/repository_submission.md)和[资料来源](docs/sources.md)；这不是 AMD 官方示例。

## 当前状态

项目仍在方案与证据核验阶段。这个公开仓库目前仅有规划文件，尚不满足赛事的完整工程包要求。没有可烧录位流、经实物确认的芯片型号或引脚约束，也没有经过仿真、综合、板测的图像处理模块。官网板卡照片和原厂资料仅用于辨认候选硬件，不能代替用户实物证据。报名审核、团队个人信息和实物照片不在此公开。
